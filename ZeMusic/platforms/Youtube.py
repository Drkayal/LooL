            ...
            for fmt in (
                ...
                if stdout:
                    ...
                    try:
                        ...
                        pass
                    # record simple success metric
                    try:
                        r = get_redis()
                        name = _cookie_basename(cookie_path)
                        await r.hset(_cookie_key(name), mapping={
                            "last_latency_ms": str(int((time.time() - start_ts) * 1000)),
                    ...
                    if last_err and ("sign in to confirm" in last_err.lower()):
                        ...
                        # Mark hard video for 15 minutes
                        if v_id:
                            try:
                                r = get_redis()
                                await r.set(f"music:v{config.CACHE_SCHEMA_VERSION}:hard:{v_id}", "1", ex=900)
                            except Exception:
                                pass
                    try:
                    ...
                    try:
                        ...
                        await r.hset(_cookie_key(name), mapping={
                            "last_latency_ms": str(int((time.time() - start_ts) * 1000)),
                            "last_err_code": ("BOT_DETECTED" if cool == 7200 else "UNKNOWN"),
